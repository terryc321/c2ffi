...
;; autowrap dump spec files to *default-pathname-defaults*
;;(autowrap:c-include "/usr/include/SDL2/SDL.h")

(c-include "/usr/include/SDL2/SDL_image.h"		   
		    :spec-path #P"/home/terry/code/c2ffi/image-sdl/spec/")


;; if it worked no errors then we can see definitions of all symbols generated
;; upper case 
(defparameter my-symbols (sort (loop for s being the external-symbols of (find-package "LIB")
				     collect s)
			       #'string< ))

;; (pretty-printer my-symbols)

;;(mapc #'describe my-symbols)q
