...
(autowrap:c-include "/usr/include/cairo/cairo.h" 
		    :spec-path #P"/home/terry/code/c2ffi/lib-cairo/spec/")
...
;; the shared library targetting and remember to drop .so ending in define-foreign-library
...
;; demo want to recreate from cairo documentation
...
;;
;; amazingly painless to write equivalent code using sly autocomplete and cl-autowrap auto generated bindings
;; cairo need to have double-float on some procedures
;; 32 integer
;; 32.0 single float
;; 32.0d0 double float
;;
(defun demo ()
  (let* ((surface (dummy:cairo-image-surface-create dummy:+cairo-format-argb32+ 240 80))
	 (cr (dummy:cairo-create surface)))
    (dummy:cairo-select-font-face cr "serif" dummy:+cairo-font-slant-normal+ dummy:+cairo-font-weight-bold+)
    (dummy:cairo-set-font-size cr 32.0d0)
    (dummy:cairo-set-source-rgb cr 0.0d0 0.0d0 1.0d0)
    (dummy:cairo-move-to cr 10.0d0 50.0d0)
    (dummy:cairo-show-text cr "hello world")
    (dummy:cairo-destroy cr)
    (dummy:cairo-surface-write-to-png surface "hello.png")
    (dummy:cairo-surface-destroy surface)))

(format t "run the demo use (demo::demo) ~%")
